...
def create_encoder_mask(seq_len, diagonal_width=1, random_prob=0.1):
    """
    Creates an encoder mask that only allows attention:
    1. Within a diagonal band of specified width
    2. At random positions with given probability
    
    Args:
        seq_len: Length of the sequence
        diagonal_width: Width of the diagonal band (1 means only main diagonal,
                       2 means main diagonal + one cell on each side, etc.)
        random_prob: Probability of allowing attention at positions outside diagonal
    ...
    # Create base mask with all connections blocked
    mask = torch.zeros(seq_len, seq_len, dtype=torch.bool)
    
    # Allow attention within diagonal band
    for i in range(-diagonal_width + 1, diagonal_width):
        diag = torch.diagonal(mask, offset=i)
        diag.fill_(True)
    
    # Add random connections outside diagonal band
    random_mask = torch.rand(seq_len, seq_len) < random_prob
    
    # Only add random connections where we don't already have diagonal connections
    mask = mask | (random_mask & ~mask)
...
def collate_fn(batch):
    ...
    specialized_encoder_mask = create_encoder_mask(encoder_inputs_padded.size(1), diagonal_width=1, random_prob=0.0)
    cross_attention_mask = (encoder_inputs_padded != PAD_index).unsqueeze(1).unsqueeze(2)
    specialized_encoder_mask = (encoder_inputs_padded != PAD_index).unsqueeze(1).unsqueeze(2) & specialized_encoder_mask
    ...
    specialized_encoder_mask = specialized_encoder_mask.to(device)
    ...
    return {
        ...
        'specialized_encoder_mask': specialized_encoder_mask,
        'cross_attention_mask': cross_attention_mask, # (batch_size, 1, 1, seq_len) or (batch_size, 1, seq_len, seq_len)
...
class InputEmbedding(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        # Convert input_tensor to LongTensor for nn.embedding
        input_tensor = input_tensor.long()
        embedding = self.embedding(input_tensor)
        return embedding * math.sqrt(self.embedding_size)
...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, embedding_size: int, n_heads: int):
        ...
        #NOTE: these are the learnable parameters that will be trained to map the input sequence
        self.W_queries = nn.Linear(embedding_size, embedding_size)
        self.W_keys = nn.Linear(embedding_size, embedding_size)
        self.W_values = nn.Linear(embedding_size, embedding_size)
        
        #NOTE this maps the final concatenated output of the attention heads to the embedding size
        self.W_out = nn.Linear(embedding_size, embedding_size)
    ...
    def forward(self, query, key, value, mask=None):
        ...
        #NOTE multiple by the learnable parameters to get the queries, keys, and values
        #NOTE (batch_size, seq_len, embedding_size) * (embedding_size, embedding_size) = (batch_size, seq_len, embedding_size)
        Q = self.W_queries(query)
        K = self.W_keys(key)
        V = self.W_values(value)
        
        #NOTE split the queries, keys, and values into n_heads
        ...
        x, attention_scores = self_attention(Q, K, V, mask)
...
class FeedForward(nn.Module):
    ...
    def __init__(self, embedding_size, hidden_size):
        ...
        self.W_1 = nn.Linear(embedding_size, hidden_size)
        self.W_2 = nn.Linear(hidden_size, embedding_size)
...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, embedding_size, n_heads, hidden_size, dropout_p=0.1):
        ...
        self.self_attention = MultiHeadedAttention(embedding_size, n_heads)
    ...
    def forward(self, x, mask):
        ...
        context, attention_scores = self.self_attention(x, x, x, mask)
        
        x = x + self.dropout_1(context)
        x = self.norm_1(x)
...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        #TODO consider applying a final layer norm here
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, embedding_size, n_heads, hidden_size, dropout_p=0.1):
        ...
        self.masked_mult_head_attention = MultiHeadedAttention(embedding_size, n_heads)
        self.norm_1 = LayerNorm()
        
        self.multi_head_attention = MultiHeadedAttention(embedding_size, n_heads)
        self.norm_2 = LayerNorm()
    ...
    def forward(self, x, encoder_outputs, src_mask, tgt_mask):
        ...
        """
        masked_attention_output, masked_attention_scores = self.masked_mult_head_attention(x, x, x, tgt_mask)
        x = x + self.dropout_1(masked_attention_output)
        x = self.norm_1(x)
        
        cross_attention_output, cross_attention_scores = self.multi_head_attention(x, encoder_outputs, encoder_outputs, src_mask)
        x = x + self.dropout_2(cross_attention_output)
        x = self.norm_2(x)
...
class Decoder(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_size, n_heads, hidden_size, n_layers, dropout_p=0.1):
        ...
        self.layers = nn.ModuleList([
            DecoderLayer(embedding_size, n_heads, hidden_size, dropout_p) for _ in range(n_layers)
        ])
    ...
    def forward(self, x, encoder_outputs, src_mask, tgt_mask):
        ...
        """
        #NOTE the input to the decoder is the target sequence and the encoder outputs where the target sequence is already embedded and positionally encoded
        
        all_masked_attention_scores = []
        all_cross_attention_scores = []
        
        for layer in self.layers:
            x, masked_attention_scores, cross_attention_scores = layer(x, encoder_outputs, src_mask, tgt_mask)
            all_masked_attention_scores.append(masked_attention_scores)
...
class Transformer(nn.Module):
    ...
    def __init__(self, src_vocab_size, tgt_vocab_size, embedding_size, n_heads, hidden_size, n_layers, dropout_p=0.1):
        ...
        self.input_embedding = InputEmbedding(src_vocab_size, embedding_size)
        self.src_positional_encoding = PositionalEncoding(embedding_size)
        self.encoder = Encoder(src_vocab_size, embedding_size, n_heads, hidden_size, n_layers, dropout_p)
        
        self.target_embedding = InputEmbedding(tgt_vocab_size, embedding_size) #NOTE Same InputEmbedding class used but with different weights
        self.tgt_positional_encoding = PositionalEncoding(embedding_size)
        self.decoder = Decoder(tgt_vocab_size, embedding_size, n_heads, hidden_size, n_layers, dropout_p)
        
        self.projection_linear_layer = nn.Linear(embedding_size, tgt_vocab_size)
    ...
    def encode(self, src_input, src_mask):
        ...
        """
        src_input = self.input_embedding(src_input)
        src_input = self.src_positional_encoding(src_input)
        encoder_outputs, all_encoder_attention_scores = self.encoder(src_input, src_mask)
        ...
        Returns:
            tuple:
                ...
                - all_cross_attention_scores (list): Cross-attention weights
                    ...
                    Each element shape: (batch_size, n_heads, tgt_seq_len, src_seq_len)
        """
        tgt_input = self.target_embedding(tgt_input)
        tgt_input = self.tgt_positional_encoding(tgt_input)
        decoder_outputs, all_masked_attention_scores, all_cross_attention_scores = self.decoder(tgt_input, encoder_outputs, src_mask, tgt_mask)
...
def train_batch(batch, transformer, optimizer, criterion, max_length=MAX_LENGTH, attention_weights_through_time=None):
    ...
    labels = batch['labels']
    specialized_encoder_mask = batch['specialized_encoder_mask']
    ...
    encoder_outputs, encoder_attention_scores = transformer.encode(encoder_inputs, specialized_encoder_mask)
    
    decoder_outputs, masked_attention_scores, cross_attention_scores = transformer.decode(decoder_inputs, encoder_outputs, cross_attention_mask, decoder_mask)
    distribution_over_words = transformer.get_predictions(decoder_outputs)
    
    #shift class dimension to be second for loss computation
    predictions = distribution_over_words.permute(0, 2, 1)
...
def translate(transformer: Transformer, sentence, src_vocab, tgt_vocab, max_length=MAX_LENGTH):
    ...
    with torch.no_grad():
        ...
        # add specialized input mask NOTE this can be removed for default encoding
        specialized_encoder_mask = create_encoder_mask(input_tensor.size(1), diagonal_width=2, random_prob=0.0).to(device)
        specialized_encoder_mask = input_mask & specialized_encoder_mask
        
        # Encode the input sentence
        encoder_outputs, encoder_attentions = transformer.encode(input_tensor, specialized_encoder_mask)
