...
class Transformer(nn.Module):
    ...
    def __init__(self, src_vocab_size, tgt_vocab_size, embedding_size, n_heads, hidden_size, n_layers, dropout_p=0.1):
        ...
        self.target_embedding = InputEmbedding(tgt_vocab_size, embedding_size) #TODO: consider name as embedding
...
def train_batch(batch, transformer, optimizer, criterion, max_length=MAX_LENGTH, attention_weights_through_time=None):
    ...
    encoder_outputs, encoder_attention_scores = transformer.encode(encoder_inputs, encoder_mask)
    
    decoder_outputs, masked_attention_scores, cross_attention_scores = transformer.decode(decoder_inputs, encoder_outputs, encoder_mask, decoder_mask)
    distribution_over_words = transformer.get_predictions(decoder_outputs)
    
    #shift class dimenssion to be second for loss computation
